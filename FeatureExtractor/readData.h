...
private:
    ...
    // return true if data retrieving was successful
    bool readNextPNG(Mat& IMAGE, Mat& DEPTH) {
        stringstream ss;
        ss << currentFrameNum;
        ...
        // Load an image from file
				IMAGE = cv::imread(fileName_RGBD,1).clone();
               
        DEPTH = cv::imread(fileName_Depth,-1).clone();
    ...
    // return true if data retrieving was successful
    bool readNextLine_RGBD(Mat& IMAGE, Mat& DEPTH) {
        string line;        
        char* line_c;
        bool file_ended = true;
...
public:
    ...
    // return true if data retrieving was successful
    bool readNextFrame(double **data, double **pos_data, int **data_CONF, int *data_pos_CONF, Mat& IMAGE, Mat& DEPTH) {
        ...
        bool status_RGBD;
        if (!READ_FROM_PNG) {
            status_RGBD = readNextLine_RGBD(IMAGE,DEPTH);
        } else {
            status_RGBD = readNextPNG(IMAGE,DEPTH);    
        }
        if (status_RGBD) {
