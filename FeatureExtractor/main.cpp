...
int main(int argc, char* argv[]) {
	...
	Mat IMAGE(cvSize(Y_RES,X_RES),CV_MAKETYPE(8,3));
	Mat DEPTH(cvSize(Y_RES,X_RES),CV_MAKETYPE(16,1));
	Mat DEPTH_CH1(cvSize(Y_RES,X_RES),CV_MAKETYPE(8,1));
	Mat DEPTH_IMAGE(cvSize(Y_RES,X_RES),CV_MAKETYPE(8,3));
	...
	for (int i=0;i<all_files.size();i++) {
		...
		bool status = DATA->readNextFrame(data, pos_data, data_CONF, pos_data_CONF, IMAGE,DEPTH);
		...
		while (status) {
			convertScaleAbs(DEPTH,DEPTH_CH1,1./40);
			merge(&vector<Mat>(3,DEPTH_CH1)[0],3,DEPTH_IMAGE);
			...
			if (started) {
				...
				if (USE_HOG) {
					...
					double * imageFeats = features_rgbd->computeFeatures(IMAGE, DEPTH_IMAGE, data, pos_data, &numImageFeats,
			...
			if (useSkeleton) {
				if ( !features_rgbd->drawSkeleton(DEPTH_IMAGE,data, pos_data) ) {
				}
			}
			// show video
			imshow("IMAGE",IMAGE);
			//imshow("DEPTH_CH1",DEPTH_CH1);
			imshow("DEPTH",DEPTH_IMAGE);
			if ( waitKey(10) == 27 )
				break;
			// read next frame
			status = DATA->readNextFrame(data, pos_data, data_CONF, pos_data_CONF, IMAGE, DEPTH);
		}
		if (useSkeleton)
